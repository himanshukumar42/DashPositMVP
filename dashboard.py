...
def load_dataset():
    house_data = pd.read_csv(f"{os.getcwd()}/data.csv")
    house = ['bedrooms', 'bathrooms', 'sqft_living', 'sqft_lot', 'floors', 'waterfront', 'view', 'condition']
    # house_df = pd.DataFrame(wine.data, columns=house_features)
    # house_df["WineType"] = [wine.target_names[typ] for typ in wine.target]
    return house, house_data


house_features, house_df = load_dataset()


def create_histogram(col_name):
    fig = px.histogram(house_df, x=col_name, color="WineType", nbins=50)
...
def create_scatter_chart(x_axis, y_axis):
    fig = px.scatter(house_df, x=x_axis, y=y_axis, color='WineType')
...
def create_pie_chart():
    wine_cnt = house_df.groupby("WineType").count()[["alcohol"]] \
...
def create_bar_chart(col_name):
    fig = px.histogram(house_df, x="WineType", y=col_name, color="WineType", histfunc="avg")
    fig.update_traces(marker={"line": {"width": 2, "color": "black"}})
    fig.update_layout(paper_bgcolor="#e5ecf6", margin={"t": 0})
    return fig


hist_drop = dcc.Dropdown(id="hist_column", options=house_features, value="alcohol",
                         clearable=False, className="text-dark p-2")
x_axis = dcc.Dropdown(id="x_axis", options=house_features, value="alcohol",
                      clearable=False, className="text-dark p-2")
y_axis = dcc.Dropdown(id="y_axis", options=house_features, value="malic_acid",
                      clearable=False, className="text-dark p-2")
avg_drop = dcc.Dropdown(id="avg_drop", options=house_features, value="malic_acid",
                        clearable=False, className="text-dark p-2")
